...
#print(info)
fileName='Ulevs_1000-001_mon.era5_195001_202105_interp.nc'
#fileName=info.get_obs(filename)
dsall = xr.open_dataset(sourceData+model+'/'+fileName)
print(dsall.level)
ds=dsall['u'][:,lev,:,:]
ds=dom.shifting_grid(ds)

print(dsall.level[lev])

variable='u200'#+str(dsall.level[lev])
units=ds.units
...
##Plotting
###
infoplot=myplot.get_var_infos('u')
cmapClim=infoplot.get('cmap')
units=infoplot.get('units')
clevsClim=np.arange(-50,55,5)#infoplot.get('levels')

cmapStd=infoplot.get('cmap_std')
clevsStd=np.arange(0,11,1)#infoplot.get('levels_std')

clevsT=np.arange(-1,1.1,0.1)#infoplot.get('levels_diff')
