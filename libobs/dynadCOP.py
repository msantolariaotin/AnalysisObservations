...
def climAnalogs_2fields_esacci(monthlyValuesX,monthlyValuesY,rmon,year,Na,Ns,Nr,iyr,fyr,uncertainty=False):
#"FieldX and FieldY are monthly fields from which to built the analogsto improve and put the original ds"
    ...
    orderList=[]
    for i in range(nyr):
        tmp=np.linalg.norm(monthlyValuesX.sel(time=str(year)).values-monthlyValuesX.sel(time=str(i)).values)
        #tmp=np.linalg.norm(X[countYear,0:nsx]-X[i,0:nsx])
        distList.append(tmp)
        orderList.append(i)
    f=list(zip(distList,orderList))
...
#    print('Taking closest Na=',Na,'analogs of the list Euc.Dist (1st element removed) \n')
    ...
    elif rmon==10 and 1994-iyr in allyears:
        index=allyears.index(1994-iyr)
        print('Removing 10-1994',len(allyears))
        print(index,rmon,'removing')
        allyears.pop(index)
        print(len(allyears),Na)
    if rmon==11 and 1994-iyr in allyears:
        index=allyears.index(1994-iyr)
        print('Removing 11-1994',len(allyears))
        print(index,rmon,'removing')
        allyears.pop(index)
        print(len(allyears),Na)
    if rmon==12 and 1994-iyr in allyears:
        index=allyears.index(1994-iyr)
        print('Removing 12-1994',len(allyears))
        print(index,rmon,'removing')
        allyears.pop(index)
        print(len(allyears),Na)
